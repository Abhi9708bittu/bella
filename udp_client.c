...
// Calculate a simple checksum for a packet
uint16_t calculateChecksum(const struct packet *pkt) {
    uint16_t sum = 0;
    for (int i = 0; i < pkt->packet_size; i++) {
        sum += pkt->data[i];
    }
    return sum;
}
// Function to add a checksum to a packet
void addChecksum(struct packet *pkt) {
...
// Send a packet
int sendPacket(struct packet *pkt) {
    addChecksum(pkt);
    return sendto(socket_fd, pkt, sizeof(struct packet), 0, ptr->ai_addr, ptr->ai_addrlen);
}
...
int main(int argc, char* argv[]) {
	...
	time1.tv_sec = 0;
	time1.tv_nsec = 3000000L;

	FILE * in = fopen("testFiles/earth.mp4","rb"); // open the video file in read mode
	...
	while (data > 0) {
		...
		for (int i = 0; i < no_of_packets; i++) {
		    ...
		    // Send a packet
    int sent_bytes = sendPacket(&packets[i]);
    if (sent_bytes < 0) {
        perror("UDP Client: sendPacket");
        exit(1);
    }
